  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Indicator`s name</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="/indikator" enctype="multipart/form-data" method="post" id="myForm">
          {{ csrf_field() }}
        <div class="form-group row">
            <label for="soal" class="col-md-4 col-form-label text-md-right">{{ __('Indicator`s name') }}</label>

            <div class="col-md-6">
                <textarea name="soal" style="font-size: 15px" class="form-control{{ $errors->has('soal') ? ' is-invalid' : '' }}" placeholder="Input the indicator" id="soal" value="{{ old('soal') }}" required></textarea>
                @if ($errors->has('soal'))
